...
var wrapper = { 
     ...
     */ 
    sandbox: function() {
        return svc.LocalServiceRegistry.createService("cko.payment.actions.sandbox.service", {
     ...
     */
    live: function() {
        return svc.LocalServiceRegistry.createService("cko.payment.actions.live.service", {
            createRequest: function (svc, args) {
                // Prepare the http service
                svc.addHeader("Authorization", util.getAccountKeys().secretKey);
                svc.addHeader("User-Agent", util.getCartridgeMeta());
