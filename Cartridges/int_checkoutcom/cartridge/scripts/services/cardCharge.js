...
var wrapper = {  
     ...
     */
    sandbox: function() {
        return svc.LocalServiceRegistry.createService("cko.card.charge.sandbox.service", {
     ...
     */
    live: function() {
        return svc.LocalServiceRegistry.createService("cko.card.charge.live.service", {
            createRequest: function (svc, args) {
                // Prepare the http service
                svc.addHeader("Authorization", util.getAccountKeys().secretKey);
                svc.addHeader("User-Agent", util.getCartridgeMeta());
