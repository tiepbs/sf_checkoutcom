...
/* Site Controller */
var SiteController = dw.system.Site.getCurrent().getCustomPreferenceValue('cko_pay_test_StorefrontController');
...
var util = {
	...
	doLog: function(dataType, gatewayData){
		if (this.getValue("cko_pay_test_debugEnabled") == true && (gatewayData)){
			var logger = Logger.getLogger("cko_test_pay_debug");
			if(logger){
				logger.debug(this._("cko.gateway.name", "cko_pay_test") + " " + dataType + " : {0}", gatewayData);
			}
		}
	},
	...
	/* return order id */
	getOrderId: function(){
		var orderId = (this.getValue('cko_pay_test_3DS')) ? request.httpParameterMap.get('reference').stringValue : request.httpParameterMap.get('reference').stringValue;
		if(orderId === null){
			orderId = session.privacy.ckoTestOrderId;
		}
	...
	getCartridgeMeta: function(catridge){
		return this.getValue("cko_pay_test_userAgent") + ' ' + this.getValue("cko_pay_test_version");
	},
	...
	getAccountKeys: function(){
		...
		keys.privateKey = this.getValue("cko_pay_test_sandbox_privateKey");
		keys.publicKey = this.getValue("cko_pay_test_sandbox_publicKey");
		keys.secretKey = this.getValue("cko_pay_test_sandbox_secreteKey");
	...
	preAuthorizeCard: function(chargeData){
		...
		var authResponse = this.gatewayClientRequest("cko.pay.test.init.sandbox.service", authData);
	...
	handleAPMChargeRequest: function(payObject, args){
		...
		case 'ideal':
			...
			var gatewayResponse = this.gatewayClientRequest("cko.pay.test.init.sandbox.service", gatewayObject);
		...
		case "sepa":
			...
			// Perform the request to the payment gateway
			var gatewayResponse = this.gatewayClientRequest("cko.pay.test.init.sources.sandbox.service", gatewayObject);
	...
	gatewayAPMObject: function(payObject, currency, args){
		...
		var chargeData = {
			"amount"			: this.getFormattedPrice(order.totalGrossPrice.value.toFixed(2)),	
...
//	 */
//	getShopInfo : function(args){
//		var address = Store.getEmail();
//		return address;
//	},
	...
	gatewayObject: function(cardData, args){
		...
		// customer object
		var customer = this.getCutomer(args);
		...
		var chargeData = {
				...
				"capture"			: this.getValue('cko_pay_test_autoCapture'),
	...
	getCaptureTime: function(){
		var captureOn = this.getValue('cko_pay_test_autoCaptureTime');
	...
	get3Ds:	function(){
		...
		// 3ds object
		var ds = {
			"enabled"				: this.getValue('cko_pay_test_3DS'),
			"attempt_n3d"			: this.getValue('cko_pay_test_non3DS')
		}
	...
	getMetadata: function(cardData){
		...
		meta = {
			udf1				: cardData.type,
			integration_data	: this.getValue('cko_pay_test_userAgent') + " " + this.getValue('cko_pay_test_version'),
